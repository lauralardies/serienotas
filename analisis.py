...
def media(): # Primero calculamos la media de cada uno de los exámenes.
...
def mediana(): # Ahora calculamos la mediana.
...
def moda(): # Por último, la(s) moda(s).
...
def var(): # Primero calculamos la varianza de cada examen realizado por los alumnos.
...
def desviacion_tipica(): # Ahora determinamos el valor de la desviación típica para cada examen
...
def cuartil(): # Por último, los cuartiles (.25, .5, .75)
...
def datos_aberrantes(): # Calculamos las notas tipificadas : 
